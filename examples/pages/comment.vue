...
<template>
  <div id="comment">
    <div>
      <button @click="unshiftParent(1)">
        unshift 1
      </button>
      <button @click="pushParent(1)">
        push 1
      </button>
      <button @click="pushParent(2)">
        concat 2
      </button>
      <button @click="unshiftParent(2)">
        merge 2
      </button>
      <button @click="totalPlus">
        total++
      </button>
    </div>
    <FlowLoader
      ...
      <ul slot-scope="{ flow }">
        ...
        >
          <div :style="{ backgroundColor: item.style.color }">
            ...
            <br><br>
            <button @click="modifyDeepValue(item)">
              {{ item.data.follow ? '已关注' : '关注' }}
            </button>
            <button @click="modifyLightValue(item)">
              {{ item.like ? '已喜欢' : '喜欢' }}
            </button>
            <button @click="deleteParent(item)">
              删除
            </button>
          ...
          >
            ...
            >
              <li
                v-for="(children, subIndex) in flow"
                :key="children.id"
              >
            ...
            <div slot="footer">
              <button @click="loadChildren(1, item.id)">
                page-1
              </button>
              <button @click="loadChildren(2, item.id)">
                page-2
              </button>
              <button @click="loadChildren(9, item.id)">
                page-9
              </button>
            </div>
          </FlowLoader>
